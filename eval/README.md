...
[`stock-vs-latest-claude-omx-v1.json`](results/stock-vs-latest-claude-omx-v1.json) is a fresh 24-call local appendix using the current checker, six cases, Claude Code with `claude-opus-4-6`, and near-stock OMX with `gpt-5.6-luna`. It compares a bounded baseline with the exact latest `SOUL.md` policy. Both harnesses produced 3/6 deterministic-clean baseline cases and 2/6 latest-policy cases; each had one baseline win, two clean ties, and three failed ties. OMX's baseline includes the native `omx exec` overlay, so it is not a pure stock Codex run. This narrow result does not establish that the baseline or policy is generally better.

The fresh appendix binds the exact policy, checker, case contract, and model invocation. It contains aggregate findings only; raw responses and runtime logs remain outside the repository. It must not be used as a general benchmark or universal response-quality claim.
